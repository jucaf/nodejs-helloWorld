                ...
                if (branchType in params.testing.predeploy.sonarQube) {
                    stage('SonarQube') {
                        ...
                        withSonarQubeEnv('sonarqube') {
                          sh "${scannerHome}/bin/sonar-scanner -X -Dsonar.testExecutionReportPaths=test-report.xml -Dsonar.projectKey=${sonar_project_key} -Dsonar.projectName=${sonar_project_name} -Dsonar.sources=."
                        }
